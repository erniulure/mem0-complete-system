...
async function fetchModels() {
  if (cachedModelsList) {
    console.log("Using cached models list:", cachedModelsList.length, "models");
    return cachedModelsList;
  }
  try {
    console.log("Fetching models from /api/config/ui/models...");
    showNotification("正在从 /api/config/ui/models 加载模型列表...", "info");
    const response = await fetch("/api/config/ui/models", {
      credentials: 'same-origin' // 确保包含 cookies
    });
    ...
    if (
      ...
      console.log("Successfully cached", cachedModelsList.length, "models");
    ...
    cachedModelsList = null; // Set to null to allow retry
    return null;
  }
}

function renderModelsInModal() {
  console.log("renderModelsInModal called, cachedModelsList:", cachedModelsList);
  if (!modelHelperListContainer) {
    console.log("modelHelperListContainer not found");
    return;
  }
  if (!cachedModelsList || !Array.isArray(cachedModelsList) || cachedModelsList.length === 0) {
    console.log("No models to render:", cachedModelsList);
    modelHelperListContainer.innerHTML =
      '<p class="text-gray-400 text-sm italic">模型列表尚未加载或为空。</p>';
    return;
  }
