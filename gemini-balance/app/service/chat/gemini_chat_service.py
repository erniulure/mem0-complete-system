    ...
    if isinstance(obj, dict):
        ...
        for key in list(obj.keys()):
            if key == 'additionalProperties':
                logger.info(f"[GEMINI_CLEAN] Removing unsupported field: {key}")
                del obj[key]
            else:
                # 递归调用
...
class GeminiChatService:
    ...
    async def generate_content(
        ...
        try:
            # 在发送请求前清理不支持的字段
            logger.info("[GEMINI_CHAT] Cleaning payload before sending to API")
            clean_unsupported_fields(payload)
            logger.info("[GEMINI_CHAT] Payload cleaned successfully")
    ...
    async def stream_generate_content(
        ...
        while retries < max_retries:
            ...
            try:
                # 在发送请求前清理不支持的字段
                logger.info("[GEMINI_CHAT] Cleaning payload before sending to API (stream)")
                clean_unsupported_fields(payload)
                logger.info("[GEMINI_CHAT] Payload cleaned successfully (stream)")
