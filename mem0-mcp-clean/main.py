    ...
    if mem0_client is None:
        ...
        try:
            ...
            if response.status_code == 200:
                # Use direct HTTP requests for local server
                return "local"
        except:
            pass
        ...
        try:
            ...
            print(f"Warning: Could not initialize mem0 client: {e}")
...
# Note: We'll set custom instructions per user if needed
...
def create_starlette_app(mcp_server: Server, *, debug: bool = False) -> Starlette:
    ...
    async def handle_sse(request: Request):
        # Extract user ID from headers and set context
        try:
