...
# 不导入MemoryClient，因为我们只使用本地HTTP模式
...
def get_mem0_client():
    """Get or create mem0 client - simplified to always use local server"""
    global mem0_client
    if mem0_client is None:
        # Always use local HTTP mode for simplicity and speed
        mem0_client = "local"
    return mem0_client
...
@mcp.tool(
    ...
    and coding patterns for future reference. Each user's data is completely isolated. Note: Processing may take 30-60 seconds due to AI analysis."""
)
async def add_coding_preference(text: str) -> str:
    ...
    try:
        ...
        if client == "local":
            # Use direct HTTP request for local server with correct format
            import httpx
            response = httpx.post(
                "http://localhost:8888/memories",
                json={
                    "messages": [{"role": "user", "content": text}],  # Correct format
                    "user_id": user_id
                },
                timeout=60.0  # Increase timeout for mem0 processing
            )
            response.raise_for_status()
            result = response.json()
            return f"Successfully added preference for user {user_id}: {text[:100]}... (Added {len(result)} memories)"
        else:
            # 云端客户端不可用，返回错误
            return f"Error: Cloud client not available and local server failed"
    ...
    try:
        ...
        if client == "local":
            ...
            response = httpx.get(
                ...
                timeout=30.0  # Reasonable timeout for GET requests
            )
            response.raise_for_status()
    ...
    try:
        ...
        if client == "local":
            ...
            response = httpx.post(
                ...
                timeout=30.0  # Reasonable timeout for search requests
            )
            response.raise_for_status()
            memories = response.json()
            return json.dumps(memories, indent=2)
